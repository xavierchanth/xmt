...
| [App shell](../architecture/app-shell.md) | Partial. Menu bar presence, an AppKit-owned lazy Settings recovery window, stable development signing, safer replacement installation, shared Accessibility presentation, single-flight actions, shared configuration resolution, and two compiled-in module lifecycles exist; there is no general registration list or per-module permission declaration API. macOS 26 may still clip XMT's status item on crowded notched menu bars, so direct launch presents Settings as the recovery surface. |
