...
Voice settings provides a confirmed **Restore Default Bindings** action scoped only to the three Voice binding lists. Confirmation atomically replaces Hold to Talk with Fn, Toggle Recording with Fn-Space, and Cancel with Fn-Escape; every other app setting remains unchanged. The action is disabled if any one of the three lists is configuration-managed. A raced management or validation failure preserves all three customized lists and publishes no partial result.
